...
describe(@"Tapstream", ^{
	...
	describe(@"Startup behavior", ^{
		...
		it(@"Fires a first-run event called <appname>-ios-install if first run", ^{
			...
			NSURL* expectedURL = [TSURLBuilder makeEventURL:config eventName:@"ios-testapp-install"];
		...
		it(@"Fires an open event on start", ^{
			...
			NSURL* expectedURL = [TSURLBuilder makeEventURL:config eventName:@"ios-testapp-open"];
